...
static NSString * const kAHKActionSheetCellIdentifier = @"Cell";
...
{
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:kAHKActionSheetCellIdentifier forIndexPath:indexPath];
...
//    [self setUpCancelButton];
    ...
    if ([UIView respondsToSelector:@selector(animateKeyframesWithDuration:delay:options:animations:completion:)]){
        ...
        [UIView animateKeyframesWithDuration:self.animationDuration delay:0 options:0 animations:^{
            ...
            [UIView addKeyframeWithRelativeStartTime:0.3f relativeDuration:0.3f animations:^{
                delayedAnimations();
            }];
        } completion:nil];
...
- (void)dismissAnimated:(BOOL)animated duration:(NSTimeInterval)duration completion:(AHKActionSheetHandler)completionHandler
{
    ...
    void(^tearDownView)(void) = ^(void) {
        ...
        for (UIView *view in @[self.blurredBackgroundView, self.tableView, self.window]) {
...
- (void)setUpTableView
{
    ...
    [tableView registerClass:[UITableViewCell class] forCellReuseIdentifier:kAHKActionSheetCellIdentifier];
