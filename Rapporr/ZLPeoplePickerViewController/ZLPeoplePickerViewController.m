...
- (void)viewDidLoad {
    ...
    [self.navigationController.navigationBar setBarTintColor:App_OrangeColor];
    ...
    [self.navigationController setFlatColorToNavigationBar:App_OrangeColor withTitleColor:[UIColor whiteColor] andFont:ROBOTO_BOLD(17)];
    
    [Utils prepareSearchBarUI:self.searchController.searchBar];
    ...
    self.navigationItem.title = self.title.length > 0 ? self.title : NSLocalizedString(@"Choose a contact", nil);
...
(nullable __kindof id<ZLPeoplePickerViewControllerDelegate>)parentViewController {
    ...
    peoplePicker.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]
