...
- (void)setPosition:(MEVFloatingButtonPosition)position {
    if (position == MEVFloatingButtonPositionBottomCenter ||
        position == MEVFloatingButtonPositionTopCenter||
...
- (UIButton *)button
{
    if (!_button) {
        ...
        UIFont * customFont = [UIFont boldSystemFontOfSize:12];
        
        UILabel *fromLabel = [[UILabel alloc]initWithFrame:CGRectMake(11, 30, 40, 20)];
        fromLabel.text = self.title;
...
//        fromLabel.center = CGPointMake( _contentView.bounds.size.width / 2, _contentView.bounds.size.height / 2);
...
{
    SEL selector1 = NSSelectorFromString(@"mev_didTapDataButton:");
    SEL selector2 = NSSelectorFromString(@"mev_animateOutView");

//    [self mev_animateOutView];

    if ([self.superview respondsToSelector:selector1]) {
        [self.superview performSelector:selector1 withObject:sender afterDelay:0.0f];
    }
    if ([self.superview respondsToSelector:selector2]) {
        [self.superview performSelector:selector2 withObject:sender afterDelay:0.0f];
    }
}
...
- (void)setupConstraints
{
    ...
    switch (_position) {
        case MEVFloatingButtonPositionBottomCenter:
            ...
            break;
        case MEVFloatingButtonPositionTopCenter:
            _contentView.center = CGPointMake(self.center.x, 70);
            break;
...
- (void)mev_validateView
{
    if ([self mev_canDisplay]) {
        ...
        // Configure button
        if (view.image) {
            [view.button setFrame:CGRectMake(0, 0, 60, 60)];
            if (view.imageColor) {
    ...
    if (self.floatingButton.buttonState == MEFloatingButtonStateDidDisappear) {
        ...
        switch (self.floatingButton.animationType) {
            ...
            case MEVFloatingButtonAnimationFromBottom:
                ...
                break;
            case MEVFloatingButtonAnimationFromTop:
                [self mev_animateInView];
                break;
            default:
                break;
