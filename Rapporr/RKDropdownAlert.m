...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [titleLabel setFont:ROBOTO_LIGHT(FONT_SIZE)];
        titleLabel.textColor = defaultTextColor;
        titleLabel.textAlignment = NSTextAlignmentCenter;
//        [self addSubview:titleLabel];
        
        //%%% message setup (the regular text below the title)
        messageLabel = [[UILabel alloc]initWithFrame:CGRectMake(X_BUFFER, 0, frame.size.width-2*X_BUFFER, 30)];
...
+(RKDropdownAlert*)alertView {
    RKDropdownAlert *alert = [[self alloc]initWithFrame:CGRectMake(0, 70, [[UIScreen mainScreen]bounds].size.width, HEIGHT)];
    return alert;
}

+(RKDropdownAlert*)alertViewWithDelegate:(id<RKDropdownAlertDelegate>)delegate yPosition:(float)yPosition
{
    RKDropdownAlert *alert = [[self alloc]initWithFrame:CGRectMake(0, yPosition, [[UIScreen mainScreen]bounds].size.width, HEIGHT)];
...
+(void)showWithDelegate:(id<RKDropdownAlertDelegate>)delegate yPosition:(float)yPosition
{
    [[self alertViewWithDelegate:delegate yPosition:yPosition]title:DEFAULT_TITLE message:nil backgroundColor:nil textColor:nil time:-1];
}

+(void)title:(NSString*)title delegate:(id<RKDropdownAlertDelegate>)delegate yPosition:(float)yPosition
{
    [[self alertViewWithDelegate:delegate yPosition:yPosition]title:title message:nil backgroundColor:nil textColor:nil time:-1];
}

+(void)title:(NSString*)title time:(NSInteger)seconds delegate:(id<RKDropdownAlertDelegate>)delegate yPosition:(float)yPosition
{
    [[self alertViewWithDelegate:delegate yPosition:yPosition]title:title message:nil backgroundColor:nil textColor:nil time:seconds];
}

+(void)title:(NSString*)title backgroundColor:(UIColor*)backgroundColor textColor:(UIColor*)textColor delegate:(id<RKDropdownAlertDelegate>)delegate yPosition:(float)yPosition
{
    [[self alertViewWithDelegate:delegate yPosition:yPosition]title:title message:nil backgroundColor:backgroundColor textColor:textColor time:-1];
}

+(void)title:(NSString*)title backgroundColor:(UIColor*)backgroundColor textColor:(UIColor*)textColor time:(NSInteger)seconds delegate:(id<RKDropdownAlertDelegate>)delegate yPosition:(float)yPosition
{
    [[self alertViewWithDelegate:delegate yPosition:yPosition]title:title message:nil backgroundColor:backgroundColor textColor:textColor time:seconds];
}

+(void)title:(NSString*)title message:(NSString*)message delegate:(id<RKDropdownAlertDelegate>)delegate yPosition:(float)yPosition
{
    [[self alertViewWithDelegate:delegate yPosition:yPosition]title:title message:message backgroundColor:nil textColor:nil time:-1];
}

+(void)title:(NSString*)title message:(NSString*)message time:(NSInteger)seconds delegate:(id<RKDropdownAlertDelegate>)delegate yPosition:(float)yPosition
{
    [[self alertViewWithDelegate:delegate yPosition:yPosition]title:title message:message backgroundColor:nil textColor:nil time:seconds];
}

+(void)title:(NSString*)title message:(NSString*)message backgroundColor:(UIColor*)backgroundColor textColor:(UIColor*)textColor delegate:(id<RKDropdownAlertDelegate>)delegate yPosition:(float)yPosition
{
    [[self alertViewWithDelegate:delegate yPosition:yPosition]title:title message:message backgroundColor:backgroundColor textColor:textColor time:-1];
}

+(void)title:(NSString*)title message:(NSString*)message backgroundColor:(UIColor*)backgroundColor textColor:(UIColor*)textColor time:(NSInteger)seconds delegate:(id<RKDropdownAlertDelegate>)delegate autoHide:(BOOL)autoHide yPosition:(float)yPosition
{
    [[self alertViewWithDelegate:delegate yPosition:yPosition]title:title message:message backgroundColor:backgroundColor textColor:textColor time:seconds autoHide:autoHide];
}
...
-(void)title:(NSString*)title message:(NSString*)message backgroundColor:(UIColor*)backgroundColor textColor:(UIColor*)textColor time:(NSInteger)seconds autoHide:(BOOL)autoHide
{
...
-(void)checkNetworkAvailablitytitle:(NSString*)title availableMessage:(NSString*)availableMessage notAvailableMessage:(NSString*)notAvailableMessage backgroundColor:(UIColor*)backgroundColor textColor:(UIColor*)textColor time:(NSInteger)seconds
{
    NSInteger time = seconds;
    titleLabel.text = title;
    
    if (availableMessage && availableMessage.length > 0) {
        messageLabel.text = availableMessage;
        if ([self messageTextIsOneLine]) {
            CGRect frame = titleLabel.frame;
            frame.origin.y = STATUS_BAR_HEIGHT+5;
            titleLabel.frame = frame;
