...
@implementation CustomExpendedText
...
- (void) customizeCell : (RPConverstionMessage*)message andIndexPath :(NSIndexPath*)indexPath andTimeStr : (NSString*)string {
    ...
    CGSize textSize = [message.message sizeWithFont:ROBOTO_LIGHT(14) constrainedToSize:CGSizeMake(210, 20000) lineBreakMode: UILineBreakModeWordWrap]; //Assuming your width is 240
    
    float heightToAdd = MAX(textSize.height, 60.0f); //Some fix height is returned if height is small or change it to MAX(textSize.height, 150.0f); // whatever best fits for you
    
    if(heightToAdd>60){
        _detailTxt.frame = CGRectMake(detailTxtFrame.origin.x, detailTxtFrame.origin.y, detailTxtFrame.size.width, heightToAdd);
    }
