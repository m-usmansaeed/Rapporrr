...
- (void)sd_setImageWithURLWithProgress:(NSURL *)url {
    
    __weak __typeof(self)weakSelf = self;

    [self sd_setImageWithURL:url placeholderImage:nil options:SDWebImageHighPriority progress:^(NSInteger receivedSize, NSInteger expectedSize) {
        
        dispatch_async(dispatch_get_main_queue(), ^{

        UIActivityIndicatorView *activity = nil;
        activity = (UIActivityIndicatorView *)[weakSelf viewWithTag:100000];
        if (!activity) {
            activity = [[UIActivityIndicatorView alloc]initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleGray];
            [activity setTag:100000];
            [activity setHidesWhenStopped:YES];
            [activity setCenter:CGPointMake(weakSelf.frame.size.width/2.0f,weakSelf.frame.size.height/2.0f)];
            [weakSelf addSubview:activity];
        }
        else {
            [activity setCenter:CGPointMake(weakSelf.frame.size.width/2.0f,weakSelf.frame.size.height/2.0f)];
        }
        ...
        UIActivityIndicatorView *activity = (UIActivityIndicatorView *)[weakSelf viewWithTag:100000];
